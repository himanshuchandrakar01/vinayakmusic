...
<div class="card border-0 shadow-sm rounded">
    ...
    <div class="card-body">
                ...
                @foreach ($carts as $key => $cartItem)
                    ...
                    <tr class="cart_item">
                        ...
                        <td class="product-total text-right">
                            <span class="pl-4 pr-0">{{ format_price(convert_price($cartItem['price'])*$cartItem['quantity']) }}</span><br>
                            <?php if($cartItem['discount']>0) { ?><span class="text-success"><small>-{{ format_price(convert_price($cartItem['discount'])) }}</small></span> <?php } ?>
                        </td>
                    </tr>
                @endforeach
        ...
        <table class="table">
            ...
            <tfoot>
                ...
                <tr class="cart-shipping">
                    ...
                    <td class="text-right">
                        <span class="font-italic">{{ single_price($customclearance) }}</span>
                    </td>
                </tr>
                ...
                <tr class="cart-shipping">
                    <th>{{translate('Total Shipping')}} <?php  if ((Session::get('country_code') == 'IN')) { echo "<br> <small>(with 18% GST)</small>"; } ?></th>
                    <td class="text-right">
        ...
        @if (addon_is_activated('club_point'))
            @if (Session::has('club_point'))
                <div class="mt-3">
                    <form class="" action="{{ route('checkout.remove_club_point') }}" method="GET" enctype="multipart/form-data">
                       {{csrf_field}}
        ...
        <hr>
        <div>
            <label for="gstin" onchange="mygst();"><input type="checkbox" id="gstin" name=""> Use GSTIN For Business Purchase (Optional)</label>
        </div>

        <div id="gst_form">
            <div class="row">
                <input type="text" class="form-control" name="gst_registration_number" placeholder="GSTIN Registration Number" onkeyup="dochanges('gst1',this.value)">
            </div>
            <br>
            <div class="row">
                <input type="text" class="form-control" name="registered_company" placeholder="Registered Company Name" onkeyup="dochanges('gst2',this.value)">
            </div>
        </div>

        <br>
        <div class="row">
                <input type="text" class="form-control" name="customer_remark" placeholder="Customer Remarks" onkeyup="dochanges('cust_rem',this.value)">
            </div>
            <br>
        <div class="row">
            <button type="button"  onclick="submitOrder(this)" class="btn-primary proceed_checkout">Proceed to Checkout</button>
        </div>
...
<script>
function mygst() {
  var checkBox = document.getElementById("gstin");
  var text = document.getElementById("gst_form");
